...
frame.def(py::init<>())
     ...
     .def(py::init([](
         const py::array_t<uchar> &color,
         const py::array_t<float> &depth,
         py::array_t<float, py::array::c_style | py::array::forcecast> roi_np,
         py::array_t<float, py::array::c_style | py::array::forcecast> pose_np,
         int frame_id,
         std::string id_str,
         py::array_t<float, py::array::c_style | py::array::forcecast> K_np,
         std::shared_ptr<YAML::Node> yml
     ) {
         auto roi_buf = roi_np.request();
         auto pose_buf = pose_np.request();
         auto K_buf = K_np.request();

         if (roi_buf.size != 4) {
           throw std::runtime_error("Frame roi must have 4 floats");
         }

         if (pose_buf.size != 16) {
           throw std::runtime_error("Frame pose must have 16 floats");
         }

         if (K_buf.size != 9) {
           throw std::runtime_error("Frame K must have 9 floats");
         }

         float *roi_ptr = static_cast<float *>(roi_buf.ptr);
         float *pose_ptr = static_cast<float *>(pose_buf.ptr);
         float *K_ptr = static_cast<float *>(K_buf.ptr);

         Eigen::Vector4f roi;
         roi << roi_ptr[0], roi_ptr[1], roi_ptr[2], roi_ptr[3];

         Eigen::Matrix4f pose;
         for (int r = 0; r < 4; ++r) {
           for (int c = 0; c < 4; ++c) {
             pose(r, c) = pose_ptr[r * 4 + c];
           }
         }
