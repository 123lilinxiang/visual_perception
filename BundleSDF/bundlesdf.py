...
class BundleSdf:
  ...
  def make_frame(self, color, depth, K, id_str, mask=None, occ_mask=None, pose_in_model=np.eye(4)):
    ...
    # Frame 第 3 个参数是 Eigen::Vector4f，必须给 float32 的 4 维数组
    roi = np.array([0, W - 1, 0, H - 1], dtype=np.float32)
    roi = np.ascontiguousarray(roi)

    pose_in_model = np.asarray(pose_in_model, dtype=np.float32).reshape(4, 4)
    pose_in_model = np.ascontiguousarray(pose_in_model)
    ...
    print(
      "[make_frame]",
      "color", color.shape, color.dtype, color.flags["C_CONTIGUOUS"],
      "depth", depth.shape, depth.dtype, depth.flags["C_CONTIGUOUS"],
      "roi", roi, roi.shape, roi.dtype, roi.flags["C_CONTIGUOUS"],
      "pose", pose_in_model.shape, pose_in_model.dtype, pose_in_model.flags["C_CONTIGUOUS"],
      "K", K.shape, K.dtype, K.flags["C_CONTIGUOUS"],
      "id", self.cnt, id_str,
      flush=True
    )
    ...
    if occ_mask is not None:
      occ_mask = np.ascontiguousarray(occ_mask, dtype=np.uint8)
  ...
  def run_global_nerf(self, reader=None, get_texture=False, tex_res=1024):
    ...
    for frame_id in frame_ids:
      if reader is not None:
        ...
        rgb = cv2.imread(reader.color_files[id], cv2.IMREAD_COLOR)
        if rgb is None:
          raise FileNotFoundError(f"Failed to read color file: {reader.color_files[id]}")

        # cv2 读出来是 BGR，NeRF 这里需要 RGB
        rgb = rgb[..., ::-1].copy()

        depth = reader.get_depth(id)
        mask = reader.get_mask(id)

        if mask is not None and mask.ndim == 3:
          mask = mask[..., 0]

        rgbs.append(np.ascontiguousarray(rgb, dtype=np.uint8))
        depths.append(np.ascontiguousarray(depth, dtype=np.float32))
        masks.append(np.ascontiguousarray((mask > 0).astype(np.uint8) * 255))
      else:
...
if __name__=="__main__":
  ...
  cfg_nerf['data_dir'] = 'BundleTrack/HO3D_v3/evaluation/MPM13'
  cfg_nerf['SPDLOG'] = 1
